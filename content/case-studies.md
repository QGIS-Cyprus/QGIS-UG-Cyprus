---
type: "page"
title: "Case Studies"
subtitle: "Local success stories"
draft: false
sidebar: true
---

{{< content-start >}}

# Case Studies

Showcasing projects, implementations, and impact stories from the QGIS community.

{{< columns-start >}}
{{< column-start class="is-flex-direction-column is-full">}}
{{< rich-box-start >}}
{{< rich-content-start themeClass="coloring-1" >}}

##### Cyprus Dams — Reservoir Monitoring Dashboard

An ongoing community project tracking water reservoir levels across Cyprus with QGIS, published as an interactive dashboard and refreshed regularly alongside official Water Development Department data. Highlighted as an example of public information at a Water Development Department workshop on managing extreme water stress, held at the Water Museum, EOA Lemesos.

[View the dashboard](https://cyprusdams.github.io/)

{{< rich-content-end >}}
{{< rich-box-end >}}
{{< column-end >}}
{{< columns-end >}}

{{< columns-start >}}
{{< column-start class="is-flex-direction-column is-full">}}
{{< rich-box-start >}}
{{< rich-content-start themeClass="coloring-1" >}}

##### Sentinel-2 Burned Area Assessment, Larnaca

Following the wildfire at the Kalo Chorio (Larnaca) firing range on 27 July 2026, Copernicus Sentinel-2 imagery was analysed in QGIS to estimate the extent of the burned area — a practical example of using open Earth observation data for post-fire assessment in Cyprus.

{{< content-image src="/img/larnaca-wildfire-burned-area-2026.jpg" alt="QGIS map showing the estimated burned area from the Kalo Chorio, Larnaca wildfire of 27 July 2026, based on Copernicus Sentinel-2 imagery" >}}

{{< rich-content-end >}}
{{< rich-box-end >}}
{{< column-end >}}
{{< columns-end >}}

{{< columns-start >}}
{{< column-start class="is-flex-direction-column is-full">}}
{{< rich-box-start >}}
{{< rich-content-start themeClass="coloring-1" >}}

##### COVID-19 Curfew Checkpoint Map

During the 2020 COVID-19 curfew, a map of Cyprus Police checkpoint locations on the roads to and from Limassol and Paphos was published to help residents plan essential travel and stay informed of restrictions — built and shared with local media outlets to reach as many residents as possible.

{{< content-image src="/img/cyprus-covid-curfew-checkpoints-map-2020.png" alt="QGIS map of Cyprus Police checkpoint locations during the 2020 COVID-19 curfew, on roads to and from Limassol and Paphos" >}}

{{< rich-content-end >}}
{{< rich-box-end >}}
{{< column-end >}}
{{< columns-end >}}

{{< columns-start >}}
{{< column-start class="is-flex-direction-column is-full">}}
{{< rich-box-start >}}
{{< rich-content-start themeClass="coloring-1" >}}

##### Rapid Test Locations Map

An interactive map of COVID-19 rapid test locations across Cyprus, built with QGIS and [qgis2web](https://plugins.qgis.org/plugins/qgis2web/), with the underlying data published openly for reuse.

[View the map](https://mariosmsk.com/rapidtestlocations/) · [Open data on GitHub](https://github.com/Mariosmsk/rapidtestlocationsdata)

{{< rich-content-end >}}
{{< rich-box-end >}}
{{< column-end >}}
{{< columns-end >}}

{{< columns-start >}}
{{< column-start class="is-flex-direction-column is-full">}}
{{< rich-box-start >}}
{{< rich-content-start themeClass="coloring-1" >}}

##### QGIS for Digital Twins of Critical Infrastructure

At the QGIS User Conference 2025 in Norrköping, Sweden, QGIS Cyprus members presented two talks developed with the KIOS Research and Innovation Centre of Excellence at the University of Cyprus:

- [QGIS as an Enabling Technology in Digital Twins for Critical Infrastructures](https://talks.osgeo.org/qgis-uc2025/talk/7J3BVM/)
- [GNOSIS: The Backbone GIS-Based Ecosystem of the Cyprus Transport Networks Digital Twin](https://talks.osgeo.org/qgis-uc2025/talk/KZDT7E/)

{{< content-image src="/img/qgis-uc2025-digital-twins-talk.jpg" alt="Marios Kyriakou presenting 'QGIS as an Enabling Technology in Digital Twins for Critical Infrastructures' at QGIS User Conference 2025, Norrköping" >}}

{{< content-image src="/img/qgis-uc2025-gnosis-talk.jpg" alt="Andreas Georgiou presenting 'GNOSIS: The Backbone GIS-Based Ecosystem of the Cyprus Transport Networks Digital Twin' at QGIS User Conference 2025, Norrköping" >}}

{{< content-image src="/img/qgis-uc2025-cyprus-team-norrkoping.jpg" alt="QGIS Cyprus members at the QGIS User Conference 2025 in Norrköping, Sweden" >}}

{{< rich-content-end >}}
{{< rich-box-end >}}
{{< column-end >}}
{{< columns-end >}}

{{< columns-start >}}
{{< column-start class="is-flex-direction-column is-full">}}
{{< rich-box-start >}}
{{< rich-content-start themeClass="coloring-1" >}}

##### ImportPhotos — an Open-Source QGIS Plugin from Cyprus

Developed by the KIOS Research and Innovation Centre of Excellence, ImportPhotos is an open-source QGIS plugin that has passed 500,000 downloads, with an active list of community contributors.

[View on GitHub](https://github.com/KIOS-Research/ImportPhotos)

{{< rich-content-end >}}
{{< rich-box-end >}}
{{< column-end >}}
{{< columns-end >}}

{{< columns-start >}}
{{< column-start class="is-flex-direction-column is-full">}}
{{< rich-box-start >}}
{{< rich-content-start themeClass="coloring-1" >}}

##### Mapping Cyprus' Past: Historical Orthophotos in QGIS

Loading historical aerial imagery from the Cyprus Department of Lands and Surveys (DLS) portal into QGIS, using the MapSwipe tool to bring in orthophotos spanning 1963 to 2014. A practical example of using QGIS with local Cypriot geospatial data.

[Watch the walkthrough](https://www.youtube.com/watch?v=1i4wBGH1fBc)

{{< rich-content-end >}}
{{< rich-box-end >}}
{{< column-end >}}
{{< columns-end >}}

{{< columns-start >}}
{{< column-start class="is-flex-direction-column is-full">}}
{{< rich-box-start >}}
{{< rich-content-start themeClass="coloring-1" >}}

Have a Cyprus-based QGIS project you'd like to showcase here? Get in touch — see below.

{{< rich-content-end >}}
{{< rich-box-end >}}
{{< column-end >}}
{{< columns-end >}}

{{< rich-box-start icon="🗂️" layoutClass="tips">}}
{{< rich-content-start themeClass="coloring-1" >}}
##### Submit a Case Study

Have a project to feature? Share details at [qgiscyprususergroup@gmail.com](mailto:qgiscyprususergroup@gmail.com).

{{< rich-content-end >}}
{{< rich-box-end >}}

{{< content-end >}}
